...
let
  ...
  systemdBootBuilder = pkgs.substituteAll rec {
    ...
    checkMountpoints = pkgs.writeShellScript "check-mountpoints" ''
      fail() {
        echo "$1 = '$2' is not a mounted partition. Is the path configured correctly?"
        exit 1
      }
    ...
    assertions = [
      {
        ...
        message = "The ESP mount point '${efi.efiSysMountPoint}' should be an absolute path";
      }
      {
        assertion = cfg.xbootldrMountPoint == null || (hasPrefix "/" cfg.xbootldrMountPoint);
        message = "The XBOOTLDR mount point '${cfg.xbootldrMountPoint}' should be an absolute path";
      }
      {
